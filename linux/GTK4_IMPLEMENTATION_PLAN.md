...
- [x] Task title renames can be confirmed with Enter or a focus-only accent-colored confirm button that disappears after saving or leaving the field.
- [x] Rename text fields use an accent bottom border only while focused/renaming, clear it on confirm/blur, avoid unsupported GTK CSS, and size it to the text.
...
- [x] Added configurable keybindings to encrypted vault settings so they sync via the encrypted cloud settings blob, not plaintext settings, and wired them into GTK shortcuts plus key-controller fallback for add/search/close/delete/toggle.
